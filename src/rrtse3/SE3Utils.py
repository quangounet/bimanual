...
def SE3Shortcut(robot, taumax, fmax, vmax, se3traj, Rlist, maxiter, 
    ...
    for it in range(maxiter):
        ...
        Tnew = RNG.random() * (t1 - t0)

        print "Shortcutting iteration", it + 1
        # print t0, t1, t1- t0       
        ...
        shortcutrtraj = lie.InterpolateSO3(R_beg, R_end, omega0, omega1, Tnew)
        ...
        (TrajString3rdDegree(t_beg, t_end, v_beg, v_end, Tnew))
        ...
        if (not isincollision):
            # attempt += 1
            # newlietraj = ReplaceTrajectorySegment(lietraj, shortcutrtraj, t0, t1)
            # newtranstraj = ReplaceTransTrajectorySegment(transtraj, shortcuttranstraj, t0, t1)
            ...
            if (ret == 1):
                ...
                if (x.resduration + SHORTCUTTHRES < Tnew): #skip if not shorter than SHORTCUTTHRES
    ...
    for it in range(maxiter):
        ...
        if (not isincollision):
           #  a,b,c = lie.ComputeSO3Constraints(shortcuttraj, taumax, discrtimestep)
            abc = TOPPbindings.RunSO3Constraints(str(shortcuttraj),constraintsstring)# discrtimestep)
