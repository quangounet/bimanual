...
import BaseBimanualManipulationPlanner2 as bbmp
from BaseBimanualManipulationPlanner2 import (FW, BW, REACHED, ADVANCED, TRAPPED,
...
class BimanualGPGMP(bbmp.BaseBimanualManipulationPlanner):
    ...
    def Plan(self, timeout):
        ...
        while (self._runningtime < timeout) and (not self._hassolution):
            ...
            self.treestart = bbmp.Tree(vstart, FW)
            self.treeend = bbmp.Tree(vgoal, BW)
    ...
    def ExtendFWFrom(self, vindex):
        ...
        if cond1 or cond2 or cond3 or cond4 or cond5:
            ...
            cnew = bbmp.Config(sols, vnear.config.tobj, qgrasps, CG1CG2CP, isurface)
            ...
            cnew = bbmp.Config(sols, Tobj, vnear.config.qgrasps, CG1CG2CP, isurface)
            vnew = bbmp.Vertex(cnew, FW, level=nextlevel)
            # TODO: code for planning TRANSFER trajectories
            [plannerstatus, trajectorystrings] = [True, ['', '']]
    ...
    def ConnectFW(self):
        ...
        for v_id in twostepsfromvfwlist:
            ...
            if curtype == TRANSIT:
                ...
                cnew = bbmp.Config(sols, Tobj, qgrasps, CG1CG2CP, cbw.isurface)
                vnew = bbmp.Vertex(cnew, FW, level=vfw.level + 1)
                ...
                cnew = bbmp.Config(sols, Tobj, qgrasps, CG1CG2CP, cfw.isurface)
                vnew = bbmp.Vertex(cnew, FW, level=vfw.level + 1)
