...
class BaseBimanualManipulationPlanner(object):
    ...
    class Config(object):
        ...
        def Clone(self):
            ...
            return  BaseBimanualManipulationPlanner.\
            Config(qrobots, tobj, qgrasps, configtype, isurface)
    ...
    class Vertex(object):
        ...
        def Clone(self):
            vnew = BaseBimanualManipulationPlanner.Vertex(self.config.Clone())
    ...
    class Tree(object):
        ...
        def Clone(self):
            tnew = BaseBimanualManipulationPlanner.Tree(None, self.treetype)
    ...
    #
    # BaseBimanualManipulationPlanner initialization
    #
    def __init__(self, robots, manipulatornames, mobj):
        ...
        self.objectplanner = ObjectPlanner(self.object)
        self.objecttracker = ObjectTracker(self.robots, manipulatornames, self.object)
    ...
    def PlanClosedChainMotion(self, qgrasps, Tinit, Tgoal, ikinit=[], refinit=[]):
        ...
        se3traj
        Rlist
        trajectorystrings : list
            A list containing trajcetory strings for both robots
        """
        ...
        # Plan the object's motion first
        [se3rrtsuccessful, se3traj, Rlist] = self.objectplanner.Plan(Tinit, Tgoal)
        if not se3rrtsuccessful:
            message = Colorize('SE(3) planner failed', 'red')
            print _funcname + message
            return [False, None, None, ['', '']]

        transtraj, rottraj = SE3Utils.TransRotTrajFromSE3Traj(se3traj)
        lietraj = Lie.SplitTraj2(Rlist, rottraj)

        
        # Solve for tracking motions
        [trackerstatus, waypointslist, timestamps] = self.objecttracker.Track\
        (lietraj, transtraj, qgrasps, self._timestep, q0list=ikinit)
        
        if not trackerstatus:
            message = Colorize('Object tracking failed', 'red')
            print _funcname + message
            return [False, se3traj, Rlist, ['', '']]

        trajectorystrings = []
        for i in xrange(len(self.robots)):
            openravetraj = Utils.ConvertWaypointstoOpenRAVETrajectory(waypointslist[i])
            trajstring = Utils.ConvertOpenRAVETrajectoryToTOPPTrajString(openravetraj)
            trajectorystrings.append(trajstring)

        return [True, se3traj, Rlist, trajectorystrings]
